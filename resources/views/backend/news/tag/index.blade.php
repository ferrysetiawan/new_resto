                    ...
                    <tbody>
                        @foreach ($tags as $no=>$item)
                            <tr>
                                ...
                                <td>{{ $item->title }}</td>
                                <td>
                                    <a href="{{ route('tags.edit', $item->id) }}" class="btn btn-warning px-4">Edit</a>
...
<script>
    $(document).ready(function () {
        $('.dataTables_filter input').after('<a href="{{ route('tags.create') }}" class="btn btn-info ms-2">Create</a>')
    });
